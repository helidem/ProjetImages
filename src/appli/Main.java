...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Image test = ImageLoader.exec("C:\\Users\\mediyo0207\\Downloads\\modifHisto.png");
    ...
    public static Image colorToGray(Image imageCouleur){
        ...
        for (int x = 0; x < imageCouleur.getXDim(); x++){
            for(int y = 0; y< imageCouleur.getYDim();y++){
                imageGris.setPixelXYBByte(x,y,0,
                        (imageCouleur.getPixelXYBByte(x,y,0) +
                                imageCouleur.getPixelXYBByte(x,y,1) +
                                imageCouleur.getPixelXYBByte(x,y,2)) / 3);
            }
        }
        return imageGris;
    }
    ...
    public static int min(Image image) {
        ...
        for(int x=0; x<image.getXDim();x++){
            for(int y=1; y<=image.getYDim();y++){
                if(min > image.getPixelXYBByte(x, y-1,0))
                    min = image.getPixelXYBByte(x, y-1,0);
            }
        }
        return min;
